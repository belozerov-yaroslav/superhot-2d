...
class Board:
    ...
    def render(self, screen):
        ...
        for i in range(self.height):
            for j in range(self.width):
                # if not past_pos is None and (j, i) == past_pos:
                #     self.board[j][i].remove(self.player_obj)
        ...
        # self.board[self.player_obj.y][self.player_obj.x].append(self.player_obj)
    ...
    def generate_field(self):
        ...
        # self.board[j][i].append(Player('pic2/pers2.png'))
        # for q in range(self.height):
        #     if q == j:
        #         break
        #     self.board[q][i].append(Creature('pic2/cell1_tile_with_lazer.jpg'))
        for i in range(10):
